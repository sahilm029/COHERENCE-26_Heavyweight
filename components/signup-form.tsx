...
export function SignupForm({ ...props }: React.ComponentProps<"div">) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem("synaptiq_users", JSON.stringify(users));

      // Auto-login: save session
      localStorage.setItem(
        "synaptiq_session",
        JSON.stringify({ name: name.trim(), email: email.trim().toLowerCase(), loggedInAt: Date.now() })
      );
  ...
  return (
    <div className="flex flex-col gap-6" {...props}>
      ...
      <div className="text-center">
        ...
        <p className="text-sm text-slate-500 mt-1">Start your journey with Synaptiq</p>
      </div>
      ...
      >
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <div className="space-y-1.5">
            <Label htmlFor="confirm-password" className="text-sm text-slate-600">Confirm Password</Label>
            <Input
              id="confirm-password"
              type="password"
              required
